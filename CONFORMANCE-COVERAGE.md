...
| portable AES v0 event and profile validation | baseline | `cts/aes/v0/aes-events-cts.v0.snapshot-0.1.json`, `cts/aes/v0/suites/01-event-and-profile-validation.json` | immutable 38-vector transport-neutral target covering record shape, complete/partial profiles, flat ancestry, nodes, attributes, identity, references, provenance/spans, header projection, WTC preservation, extensions, and semantic diagnostics; JavaScript and Rust pass independently | AltoPelago AES Draft 0 conformance candidate |
| Telex v0 syntax, canonicalization, and format limits | baseline | `cts/telex/v0/telex-cts.v0.snapshot-0.1.json`, `cts/telex/v0/suites/01-syntax-and-canonicalization.json`, `cts/telex/v0/suites/02-resource-limits.json` | immutable 50-vector encoding target covering framing, escaping, field order, canonical bytes, compact datatype expansion, order preservation, and inclusive format-limit boundaries; JavaScript and Rust pass independently | AltoPelago Telex Draft 0 conformance candidate |
| AEON-to-portable-AES projection | experimental | `cts/aes/v1/aes-cts.v1.next.json`, `cts/aes/v1/suites/01-baseline.json` through `cts/aes/v1/suites/08-portable-value-payloads.json` | mutable 82-vector source-projection target covering every scalar representation kind and canonical string payload, source-spelling normalization, distinct clone/pointer references, indexed order, flat attributes, node expansion, all four structural-identity locations, exact-source UTF-8 provenance spans, and explicit structured/shorthand/empty header planes with fail-closed conflict, default body-only behavior, and quoted `aeon:*` payload disambiguation; passes in TypeScript, Rust, Python, and PHP | AEON implementation AES projection suites |
| structural identity preservation, validity, and document-wide uniqueness | baseline | `cts/core/v1/suites/15-structural-identity.json` | covers named bindings, anonymous container heads, portable identifier syntax, duplicate rejection, and rejection on attribute-entry and literal node heads | `overview/proposals/archived/0007-structural-identity.md`, implementation parser and canonicalization suites |
...
| consumer-selected Aeonic resource-limit boundaries | experimental | `cts/core/v1/core-cts.v1.next.json`, `cts/core/v1/core-limits-cts.v1.next.json`, `cts/core/v1/suites/16-aeonic-limit-boundaries.json`, `cts/core/v1/suites/17-next-limit-semantics.json` | consolidated next Core target covers at-limit acceptance and one-over rejection for all 16 AEON parsing/compilation counters and replaces four superseded released-snapshot expectations without mutating that snapshot; both the complete 265-vector target and the smaller limit-only manifest pass in TypeScript, Rust, Python, and PHP | AltoPelago AES limits contract and four implementation suites |
