...
- structural identity preservation, validity, and document-wide uniqueness
...
- consumer-selected Aeonic resource-limit boundaries
- clone finalization and transport framing limit boundaries
...
| portable AES v1 event and profile validation | baseline | `cts/aes/v1/aes-events-cts.v1.snapshot-0.1.json`, `cts/aes/v1/suites/01-event-and-profile-validation.json` | immutable 38-vector transport-neutral target covering record shape, complete/partial profiles, flat ancestry, nodes, attributes, identity, references, provenance/spans, header projection, WTC preservation, extensions, and semantic diagnostics; JavaScript and Rust pass independently | AltoPelago AES v1 conformance candidate |
| Telex v1 syntax, canonicalization, and format limits | baseline | `cts/telex/v1/telex-cts.v1.snapshot-0.1.json`, `cts/telex/v1/suites/01-syntax-and-canonicalization.json`, `cts/telex/v1/suites/02-resource-limits.json` | immutable 50-vector encoding target covering framing, escaping, field order, canonical bytes, compact datatype expansion, order preservation, and inclusive format-limit boundaries; JavaScript and Rust pass independently | AltoPelago Telex v1 conformance candidate |
| Film v1 binary framing, records, transcoding, and format limits | baseline | `cts/film/v1/film-cts.v1.snapshot-0.1.json`, `cts/film/v1/suites/01-framing-and-canonicalization.json` through `cts/film/v1/suites/03-resource-limits.json` | immutable 72-vector reader target covering binary framing, canonical encodings, every portable record field, Telex equivalence, syntax diagnostics, truncation, and inclusive resource-limit boundaries; JavaScript and Rust pass independently; durable writer enablement remains deferred | AltoPelago Film v1 conformance candidate |
| AEON-to-portable-AES projection | baseline | `cts/aes/v1/aes-cts.v1.snapshot-0.3.json`, `cts/aes/v1/suites/01-baseline.json` through `cts/aes/v1/suites/08-portable-value-payloads.json` | immutable, hash-pinned 82-vector source-projection target covering every scalar representation kind and canonical string payload, source-spelling normalization, distinct clone/pointer references, indexed order, flat attributes, node expansion, all four structural-identity locations, exact-source UTF-8 provenance spans, and explicit structured/shorthand/empty header planes with fail-closed conflict, default body-only behavior, and quoted `aeon:*` payload disambiguation; passes in TypeScript, Rust, Python, and PHP | AEON implementation AES projection suites |
| structural identity preservation, validity, and document-wide uniqueness | baseline | `cts/core/v1/suites/15-structural-identity.json` | covers named bindings, anonymous container heads, portable identifier syntax, duplicate rejection, and rejection on attribute-entry and literal node heads | `roadmap/aeon/proposals/archive/0007-structural-identity.md`, implementation parser and canonicalization suites |
...
- experimental Instruction parse, lower, plan, and target-surface behavior
