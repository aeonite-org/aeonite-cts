...
| AEON-to-portable-AES projection | experimental | `cts/aes/v1/aes-cts.v1.next.json`, `cts/aes/v1/suites/01-baseline.json` through `cts/aes/v1/suites/06-source-provenance-spans.json` | mutable 73-vector source-projection target covering values, indexed order, flat attributes, references, node expansion, all four structural-identity locations, and exact-source UTF-8 provenance spans across ASCII, non-ASCII, combining, astral, BOM/CRLF, and absent-source cases; passes in TypeScript, Rust, Python, and PHP | AEON implementation AES projection suites |
