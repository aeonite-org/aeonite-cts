...
| AEON-to-portable-AES projection | experimental | `cts/aes/v1/aes-cts.v1.next.json`, `cts/aes/v1/suites/01-baseline.json` through `cts/aes/v1/suites/07-header-projection.json` | mutable 79-vector source-projection target covering values, indexed order, flat attributes, references, node expansion, all four structural-identity locations, exact-source UTF-8 provenance spans, and explicit structured/shorthand/empty header planes with fail-closed conflict, default body-only behavior, and quoted `aeon:*` payload disambiguation; passes in TypeScript, Rust, Python, and PHP | AEON implementation AES projection suites |
