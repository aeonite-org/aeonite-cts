...
| experimental Instruction parse, lower, plan, and target-surface behavior | experimental | `cts/sansa/v1/suites/08-instruction.json` | covers direct parse canonicalization, candidate-relative lowering, lowered operation planning with source provenance, lower-versus-plan diagnostics, one-intent parse rejection, and target-surface representability checks after successful planning |
